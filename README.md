<p align="center">
<img src="https://latex.codecogs.com/svg.image?\huge&space;\mathrm{{0{\mathrm{{\color{Red}x}}\mathbf{00-}\color{Red}dl}}" title="\mathrm{NiR{\!\mathrm{{\color{Red}\sum}}\!\!\!\mathbf{v}il}}" width="180x" </p><br><br/>  
<div dir="rtl" align="center">
   
**[#️⃣ مشاهده لیست فایل‌های دانلود شده](./Links.md#)**

</div><br><br/>  

> [!NOTE]
>
> ##
>  
> **با این پروژه می‌توانید بدون نیاز به هیچ سرور یا نرم‌افزاری، محدودیت‌های تحمیلی را دور بزنید.**  
> - لینک ویدیوهای یوتیوب (پرسرعت و بدون نیاز به کوکی)
> - لینک‌های مستقیم از وب‌سایت‌ها  
> - لینک فایل‌های بخش releases گیت‌هاب  
> - لینک‌های سایت‌ Bunkr  
> - لینک‌های مستقیم صادر شده توسط ربات‌های فایل به لینک تلگرامی [^1]
> 
> لینک‌ها را به این ربات بدهید تا در چند ثانیه آن‌ها را دانلود کرده و مستقیما روی اکانت گیت‌هاب شما ذخیره کند. سپس می‌توانید فایل‌ها را با اینترنت داخلی بدون نیاز به vpn و با سرعت بالا دانلود کنید.  
>
> <br/>
> 

> [!NOTE]
>
> ##
> 
> **⚠️ تغییرات و بهینه‌سازی‌های اخیر (نسخه جدید):**
> 
> - **سرعت اجرای فوق‌العاده بالا:** با پیاده‌سازی تکنیک *Sparse Checkout*، حتی اگر فولدر دانلودهای شما گیگابایت‌ها حجم داشته باشد، مرحله آماده‌سازی ربات از چند دقیقه به **کمتر از ۵ ثانیه** کاهش یافته است.
> - **حق انتخاب برای دانلود یوتیوب:** منوی جدیدی اضافه شده تا کاربران کوکی‌دار بتوانند بدون معطلی و مستقیماً با `yt-dlp` دانلود کنند، و کاربران موبایل بتوانند روش Fast API (بدون کوکی) را انتخاب نمایند.
> - **تکه‌تکه کردن ویدیوها (Smart Splitting):** با الهام از ایده کاربران، حالا می‌توانید انتخاب کنید فایل‌های حجیم ویدیویی (بالای ۹۵ مگابایت) به جای فشرده شدن، توسط سیستم به بخش‌های ۵ دقیقه‌ای و **قابل پخش (MP4)** تبدیل شوند. هر بخش به طور مستقل قابل پخش خواهد بود. 
> - **سیستم متغیر نصب ابزارها (Lazy Install):** ابزارهایی مانند ffmpeg و 7z تنها زمانی نصب می‌شوند که به آن‌ها نیاز باشد، در نتیجه دانلود فایل‌های ساده در سریع‌ترین زمان ممکن انجام می‌شود.
> - **پشتیبانی از چندین لینک:** می‌توانید لینک‌ها را با فاصله `Space`، کاما `,` یا خط جدید `Enter` جدا کنید تا ربات همه را یک‌جا دانلود کند.
>
> • اضافه شدن ربات تلگرامی برای پروژه
> 
> <br><br/>
>  
<br/>   

> [!TIP]
>
> ##
> 
> **🌐 چه لینک‌هایی پشتیبانی می‌شوند؟**
> 
> این ربات به صورت هوشمند نوع لینک ورودی شما را تشخیص داده و بهترین موتور دانلود را برای آن انتخاب می‌کند:  
> - 🔴 **یوتیوب (YouTube):** پشتیبانی از دانلود مستقیم با دو متد مجزا: ۱- از طریق API سریع (بدون کوکی) ۲- از طریق `yt-dlp` پایدار (نیازمند کوکی).
> - 🗃️ **سایت Bunkr:** پشتیبانی کامل از تمامی دامنه‌های بانکر (bunkr.si, bunkr.ru, bunkr.cr و...) با استفاده از اسکریپت اختصاصی پایتون جهت دور زدن محدودیت‌ها.  
> - 📱 **شبکه‌های اجتماعی و استریم:** پشتیبانی از لینک‌های Twitch, Reddit, Vimeo, SoundCloud و صدها سایت دیگر (پشتیبانی شده توسط موتور قدرتمند `yt-dlp`).  
> - ⚡ **لینک‌های مستقیم (Direct Links):** دانلود هر نوع فایل (MP4, MP3, ZIP, APK و...) از هر وب‌سایتی. این لینک‌ها توسط موتور `Aria2` (با قابلیت اتصال چندگانه) دانلود می‌شوند تا از حداکثر پهنای باند استفاده شود.  
> - 🤖 **ربات‌های تلگرامی:** لینک‌های مستقیمی که توسط ربات‌های "فایل به لینک" تلگرامی ساخته می‌شوند.   
> - 🐙 **گیت‌هاب:** لینک فایل‌های نصبی موجود در بخش Releases پروژه‌های گیت‌هاب.
>
> <br/>
> 
<br><br/>

## ⚙️ راهنمای نصب و راه‌اندازی (فقط برای بار اول)

برای داشتن این ربات روی اکانت گیت‌هاب خود، مراحل زیر را یک‌بار انجام دهید:

### مرحله ۱: کپی کردن پروژه (Fork)
1. در بالای همین صفحه، روی دکمه **Fork** کلیک کنید تا یک کپی از این پروژه در اکانت شما ساخته شود.
2. نام دلخواه خود را وارد کرده و دکمه **Create fork** را بزنید.

<br/>

### مرحله ۲: دادن دسترسیِ ذخیره فایل به ربات
1. در مخزن خودتان (پروژه‌ای که فورک کردید)، به تب **Settings** بروید.
2. از منوی سمت چپ، روی **Actions** و سپس **General** کلیک کنید.
3. تا انتهای صفحه اسکرول کرده تا به بخش **Workflow permissions** برسید.
4. گزینه **Read and write permissions** را انتخاب کرده و دکمه **Save** را بزنید.

<br/>	 

### مرحله ۳: روشن کردن ربات (Enable Actions)
1. به تب **Actions** در بالای صفحه بروید.
2. یک دکمه بزرگ سبز رنگ می‌بینید که نوشته:  
`I understand my workflows, go ahead and enable them`  

روی آن کلیک کنید تا اکشن‌ها برای شما فعال شوند.

<br/>   

## 🍪 مرحله ۴ (اختیاری اما توصیه شده): تنظیمات کوکی یوتیوب  
در متد Fast API ربات نیازی به کوکی ندارد. اما اگر متد `yt-dlp` را انتخاب کنید (به خصوص برای ویدیوهای دارای محدودیت سنی)، نیاز به تنظیم کوکی دارید:

1. **نصب افزونه:** ابتدا افزونه `Get cookies.txt LOCALLY` را روی مرورگر کامپیوتر خود نصب کنید:  
   - [🌐 لینک دانلود برای مرورگر Chrome](https://chromewebstore.google.com/detail/get-cookiestxt-locally/cclelndahbckbenkjhflpdbgdldlbecc)  
   - [🌐 لینک دانلود برای مرورگر Firefox](https://addons.mozilla.org/en-US/firefox/addon/get-cookies-txt-locally/)
2. **دریافت کوکی:** در مرورگر خود وارد سایت YouTube شوید و به حساب کاربری خود لاگین کنید (پیشنهاد می‌شود برای احتیاط از یک اکانت فرعی Gmail استفاده کنید).
3. روی آیکون افزونه‌ای که نصب کردید کلیک کنید و گزینه **Export as** را بزنید. فایل را در مسیر دلخواه ذخیره کنید.  
4. **ثبت در گیت‌هاب:** فایل را با Notepad باز کرده و **کل محتوای آن** را کپی کنید.
5. به گیت‌هاب برگشته و به این مسیر بروید:  
   **Repository Settings** → **Secrets and variables** → **Actions**  
6. روی دکمه **New repository secret** کلیک کنید.
7. در کادر **Name** بنویسید:

```
YOUTUBE_COOKIES
```

8. در کادر Secret، محتوای کامل فایل txt که در قدم چهارم کپی کرده بودید را Paste کرده و دکمه **Add secret** را بزنید.

<hr/><br/>

## 📋 راهنمای استفاده و دانلود فایل‌ها

### چگونه دانلود کنم؟
1. به تب **Actions** بروید.
2. از منوی سمت چپ روی **📥 0x00 dl** کلیک کنید. (اگر کاربر موبایل هستید در تب اکشن روی **all workflows** کلیک کرده و **0x00 dl** را انتخاب کنید)
3. در سمت راست، روی دکمه **Run workflow** کلیک کنید.
4. منویی برای شما باز می‌شود:
   - **لینک دانلود:** لینک ویدیو یا فایل خود را وارد کنید. (امکان وارد کردن چندین لینک به صورت همزمان وجود دارد).
   - **روش دانلود یوتیوب (YouTube Method):** انتخاب کنید که با API سریع دانلود شود (برای موبایل) یا مستقیماً با `yt-dlp` (اگر کوکی ذخیره کرده‌اید).
   - **روش تقسیم فایل‌ها (Split Method):** انتخاب کنید فایل‌های بالای ۹۵ مگابایت به پارت‌های فشره‌ی `001.` تبدیل شوند یا در صورت امکان به پارت‌های ویدیویی (MP4) قابل پخش.
   - **کیفیت:** کیفیت مد نظر را انتخاب کنید (مخصوص یوتیوب).
   - **رمز (اختیاری):** رمزگذاری فایل خروجی.
5. روی دکمه سبز رنگ **Run workflow** کلیک کرده و صبر کنید تا دانلود تمام شود و تیک سبز رنگ ظاهر شود. 

<br/>	 

### چگونه فایل را دریافت کنم؟
#### دسترسی سریع (پیشنهاد می‌شود)
1. در تب **Code** فایل [#️⃣ Links.md](./Links.md#) را باز کنید. لینک‌ دانلود تمام فایل‌های شما با تاریخ و ساعت دقیق آنجاست. با کلیک روی هرکدام پروسه دانلود با بالاترین سرعت شروع خواهد شد.

#### دسترسی نرمال
1. به تب **Code** (صفحه اصلی پروژه) بروید.
2. وارد پوشه **dl** شوید. فایل دانلود شده‌ی شما با همان اسم فایل آنجاست.
3. روی فایل مورد نظر کلیک کنید.
4. در سمت راست، روی دکمه **سه نقطه (...)** کلیک کنید.
5. برای دانلود مستقیم، روی **Download** کلیک کنید. 
6. و یا اگر از دانلود منیجر استفاده می‌کنید، مجددا روی دکمه سه نقطه کلیک کرده و روی گزینه `View` راست‌کلیک کرده و گزینه `Copy link address` را بزنید تا لینک خام (Raw) کپی شود. (اگر کاربر موبایل هستید انگشت خود را چند ثانیه روی گزینه‌ی `view` نگه‌دارید تا منوی آپشن ظاهر شود سپس روی گزینه‌‌ی `copy link address` کلیک کنید.)

<br><br/> 

## استفاده از طریق ربات تلکرام  
همچنین برای استفاده از این پروژه میتونید از این ربات فوق‌العاده‌‌ی تلگرامی استفاده کنید:  

- [@RGitUpBot][1]  

توضیحات کامل در داخل رپو نوشته شده:  

- [Arshiacomplus/SandBox][2]

<br/> 

> [!WARNING]
>
> ##
> 
> **قوانین و محدودیت‌های گیت‌هاب:**
> 1. **محدودیت حجم فایل:** بخاطر محدودیت‌ ۱۰۰ مگابایتی گیت‌هاب، فایل‌های بزرگتر از `95 MB` (بسته به روش انتخابی شما) یا به پارت‌های ویدیویی، و یا به صورت خودکار به فایل‌های فشرده تکه‌تکه با فرمت استاندارد (مثلاً `video.7z.001` و `video.7z.002`) تبدیل می‌شوند. برای استخراج نوع فشرده، کافیست همه پارت‌ها را دانلود کرده و فقط روی فایل `.001` کلیک راست و Extract کنید.   
> در اندروید می‌توان توسط اپلیکیشن‌هایی مانند [ZArchiver](https://play.google.com/store/apps/details?id=ru.zdevs.zarchiver) یا [MiXplorer][3] فایل‌های تکه‌تکه شده را استخراج نمود.  
> در ویندوز برای این منظور می‌توان از [7-Zip][5] و یا [WinRAR][4] استفاده کرد. [^4]   
> 2. **محدودیت حجم کل ریپازیتوری:** گیت‌هاب برای کل فضای یک پروژه محدودیت (حدود ۳ تا ۵ گیگابایت) دارد. **لطفاً هر چند وقت یک‌بار فایل‌های قدیمی را از پوشه `dl` پاک کنید** تا ریپازیتوری شما read-only نشود.
>
> <br/>

> [!NOTE]
>
> ##
> 
> اکشن **apk downloader** صرفا برای تسته و قابل استفاده نیست، سیستم تشخیص ربات گوگل‌پلی سفت و سخت درخواست‌های اکشن گیت‌هاب رو بلاک می‌کنه!! نه تنها خود گوگل بلکه اکثر Mirrorها اجازه دانلود با اکشن گیت‌هاب رو نمیدن، کد فعلی صرفا از سایت apk pure فقط فایل‌های نصبی با معماری پردازنده `armeabi-v7a` رو پشتیبانی می‌کنه که خب این فایل برای دستگاه‌های اندروید قدیمی هست و به درد ما نمی‌خوره خیلی، فقط از هیچی بهتره، اگه خیلی لنگ مونده بودین میتونید پکیج‌نیم یا لینک گوگل‌پلی اون برنامه رو بدید براتون دانلود کنه، برنامه در هر فرمتی که باشه اونو تبدیل به apks استاندارد می‌کنه در خروجی، که با اپ‌هایی مثل [MT Manager] یا [SAI] به راحتی قابل نصب هستن، اگه ایده بهتری داشتم حتما اونو پیاده کرده و اطلاع میدم.
>
> شماهم اگه ایده و نظری داشتید خوشحال میشم با ما درمیون بگذارید، لینک گروه تلگرامی ما:
> 
> [NiREvil 0x00]
>
> <br/>


> [!IMPORTANT]
>
> ##
>  
> با تشکر از توسعه دهنده اصلی [^2] جهت توسعه‌ی این پروژه‌‌ی خلاقانه‌ 
>
> ##
> 
> و با تشکر از عرشیای عزیز بابت توسعه ربات فوق‌العاده‌ی تلگرام 🩶 [^3]
>
> ##
> 
> قدردانی ویژه از دوست عزیز [BraveVahid][6] بابت [مشارکت در کدنویسی و پیشنهاد ایده‌ی جذابِ "تکه‌تکه کردن ویدیوهای حجیم به بخش‌های قابل استریم"][7] که در این نسخه پیاده‌سازی شده است. 🤝
> 
> <br><br/>
> <br/>
<br><br/>	 



[^1]: [نمونه‌ای از ربات‌های فایل به لینک تلگرامی](https://t.me/DownloadYarbot)

[^2]: [maanimis](https://github.com/maanimis/github-sandbox)

[^3]: [Arshia Complus](https://telegram.me/Arshiacomplus)

[^4]: [how-to extract split zip files](https://github.com/arshiacomplus/sandbox/#-how-to-extract-split-zip-files-zip001-zip002)

[1]: https://telegram.me/RGitUpBot
[2]: http://github.com/arshiacomplus/sandbox
[3]: https://www.farsroid.com/mixplorer-android/
[4]: https://www.win-rar.com
[5]: https://www.7-zip.org
[6]: https://github.com/BraveVahid
[7]: https://github.com/nscl5/sandbox/pull/18#issue-4415115238
[NiREvil 0x00]: https://t.me/NiREvil_GP
[MT Manager]: https://c147793.parspack.net/c147793/MT-Manager_v2.26.3.apk
[SAI]: https://c147793.parspack.net/c147793/SAI_v4.5.zip
